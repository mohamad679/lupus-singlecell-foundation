...
def test_stage5_f001_sets_stage5_current_project_state_without_modeling_authorization():
    ...
    assert "current_feature: STAGE5-F001" in state
    assert "modeling_readiness: blocked_pending_explicit_modeling_approval" in state
    assert "status: in_progress" in block
    assert "branch: feat/stage5-modeling-approval-scaffold" in block
...
def test_stage5_f001_current_feature_document_records_scaffold_scope():
    ...
    assert "Branch: `feat/stage5-modeling-approval-scaffold`" in current_feature
    assert "Stage 5 - Modeling stage approval and execution planning" in current_feature
    assert "Stage 5 has started, but modeling is not authorized." in current_feature
